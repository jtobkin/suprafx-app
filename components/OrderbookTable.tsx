...
// FIX: "matched" is now the first step (replacing legacy "open")
// Legacy "open" status treated as step 0 for backwards compat
...
function InlineTimer({ deadline }: { deadline: string }) {
  ...
  useEffect(() => {
    const iv = setInterval(() => setRemaining(Math.max(0, new Date(deadline).getTime() - Date.now())), 1000);
                            ...
                            {nodes.map((nodeId, ni) => {
                              ...
                              return (
                                <div key={nodeId} className="flex items-center gap-3 px-3 py-1.5 text-[11px]"
                                  style={{
                                    borderBottom: ni < 4 ? "1px solid var(--border)" : "none",
...
function ActiveTrade({ trade, onUpdate, rfq, tradeQuotes, agents, supraAddr }: {
  trade: Trade; onUpdate: () => void; rfq?: RFQ; tradeQuotes?: Quote[]; agents?: Agent[]; supraAddr?: string
}) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/link-address?supra=${encodeURIComponent(counterpartySupraAddr)}`)
      ...
      .then(data => {
        ...
        if (chainMatch) setResolvedRecipient(chainMatch.linked_address);
        ...
        else if (isSupraChain) setResolvedRecipient(counterpartySupraAddr);
        else setResolvedRecipient(null);
      })
  ...
  const sendOnChain = async (): Promise<string> => {
    if (!resolvedRecipient) throw new Error("No recipient address resolved");
    if (isEvmChain) {
      const toAddr = resolvedRecipient.startsWith("0x") ? resolvedRecipient : "0x742d35Cc6634C0532925a3b844Bc9e7595f2bD1e";
      return await sendSepoliaEth(toAddr, "0x" + BigInt(10000000000000).toString(16));
    } else if (isSupraChain) {
      return String(await sendSupraTokens(resolvedRecipient, 0.001));
    }
  ...
  const settle = async () => {
    setLoading(true);
    setLogs([]);
    if (!canSettle) { addLog("No wallet connected for " + chainName(myChain), "var(--negative)"); setLoading(false); return; }
    if (!isValidRecipient) { addLog("Cannot settle: counterparty has no linked address on " + chainName(myChain), "var(--negative)"); setLoading(false); return; }
    try {
      ...
      addLog((isEvmChain ? "Waiting for Ethereum block confirmation" : "Waiting for Supra block confirmation") + "...");
      ...
      if (result.status === "failed" || result.success === false) {
        ...
        onUpdate(); setLoading(false); return;
      }
      ...
      if (result.error && result.status !== "failed") addLog("Warning: " + result.error, "var(--warn)");
      onUpdate();
    } catch (e: any) {
      if (e.code === 4001 || e.message?.includes("rejected")) addLog("Transaction rejected", "var(--negative)");
  ...
  // Taker needs to act when status is "open" or "matched" (both mean same phase)
  ...
  return (
    <div className="px-4 py-3" style={{ borderLeft: "3px solid var(--accent)", background: "var(--bg-raised)" }}>
      {/* Deal Summary */}
      {(() => {
        const quoteClean = trade.pair.split("/")[1]?.replace("fx", "") || "";
        ...
        const isMineDisplay = trade.taker_address === supraAddr;
        return (
          <div className="mb-3">
            <div className="grid grid-cols-3 gap-6 mb-3 px-1">
              <div>
                  ...
                  <>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="mono text-[13px]" style={{ color: "var(--t2)" }}>
                        {fmtRate(askingPrice)} {quoteClean}
                      </span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="text-[11px]" style={{ color: "var(--t3)" }}>Filled:</span>
                      <span className="mono text-[13px] font-semibold" style={{ color: "var(--t1)" }}>{fmtRate(trade.rate)} {quoteClean}</span>
                      {priceDiff !== null && (
                ...
                <div className="flex items-center gap-2 mt-0.5">
                  ...
                  <span className="mono text-[13px]" style={{ color: "var(--positive)" }}>{fmtRate(notional)} {quoteClean}</span>
                </div>
              </div>
              <div>
                ...
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="mono text-[12px]" style={{ color: isMineDisplay ? "var(--accent)" : "var(--t2)" }}>
                    {isMineDisplay ? "You" : shortAddr(trade.taker_address)}
                  </span>
                  {takerAgent && <span className="mono text-[10px] px-1 py-0.5 rounded" style={{ background: "var(--surface-2)", color: Number(takerAgent.rep_total) >= 4 ? "var(--positive)" : "var(--t3)" }}>* {Number(takerAgent.rep_total).toFixed(1)}</span>}
                </div>
                <div className="flex items-center gap-2">
                  <span className="text-[11px] w-12" style={{ color: "var(--t3)" }}>Maker:</span>
                  <span className="mono text-[12px]" style={{ color: "var(--t2)" }}>
                    {trade.maker_address === "auto-maker-bot" ? "SupraFX Bot" : shortAddr(trade.maker_address)}
                  </span>
                  {makerAgent && <span className="mono text-[10px] px-1 py-0.5 rounded" style={{ background: "var(--surface-2)", color: Number(makerAgent.rep_total) >= 4 ? "var(--positive)" : "var(--t3)" }}>* {Number(makerAgent.rep_total).toFixed(1)}</span>}
                </div>
              </div>
              <div>
      ...
      {/* TAKER SENDS FIRST (open or matched) */}
      {takerNeedsToAct && (
        <div>
          {trade.taker_deadline && (
            <CountdownTimer deadline={trade.taker_deadline}
          ...
          {isTaker ? (
            <>
              ...
              {!canSettle && <div className="text-[12px] mb-2" style={{ color: "var(--warn)" }}>{isEvmChain ? "Connect MetaMask or StarKey EVM to settle" : "Connect StarKey to settle"}</div>}
              {canSettle && !recipientLoading && !isValidRecipient && <div className="text-[12px] mb-2" style={{ color: "var(--warn)" }}>Counterparty has not linked a {isEvmChain ? "Sepolia" : "Supra"} address yet</div>}
              {recipientLoading && <div className="text-[12px] mb-2" style={{ color: "var(--t3)" }}>Resolving counterparty address...</div>}
              <div className="flex items-center gap-2">
      ...
      {/* TAKER SENT */}
      {trade.status === "taker_sent" && trade.taker_deadline && <CountdownTimer deadline={trade.taker_deadline} label="Verification deadline:" />}
      {trade.status === "taker_sent" && (
        <div className="flex items-center gap-2 text-[13px]" style={{ color: "var(--t2)" }}>
          <Spinner color="var(--accent)" /> Committee verifying taker TX...
          {trade.taker_tx_hash?.startsWith("0x") && (
            <a href={`https://sepolia.etherscan.io/tx/${trade.taker_tx_hash}`} target="_blank" className="font-mono text-[14px] ml-1" style={{ color: "var(--accent-light)" }}>Etherscan</a>
          )}
        </div>
      )}
      ...
      {trade.status === "taker_verified" && (
        <div>
          ...
          {trade.maker_deadline && (
            <CountdownTimer deadline={trade.maker_deadline}
          ...
          {(isMaker || isBot) ? (
            ...
            <div className="flex items-center gap-2 text-[13px]" style={{ color: "var(--t2)" }}>
              <Spinner color="var(--positive)" /><span>Waiting for maker to settle...</span>
            </div>
          )}
        </div>
      )}
      ...
      {/* MAKER SENT */}
      {trade.status === "maker_sent" && trade.maker_deadline && <CountdownTimer deadline={trade.maker_deadline} label="Verification deadline:" />}
      ...
      {trade.status === "failed" && (
        <div className="px-2.5 py-2 rounded text-[13px]" style={{ background: "rgba(239,68,68,0.06)", color: "var(--negative)" }}>
          Trade failed - check Vercel logs. The maker bot may not be funded or the Supra RPC may be down.
        </div>
      )}
      ...
      {trade.status === "settled" && (
        <div className="flex items-center gap-3">
          {trade.taker_tx_hash && (
            <a href={trade.source_chain === "sepolia" ? `https://sepolia.etherscan.io/tx/${trade.taker_tx_hash}` : `https://testnet.suprascan.io/tx/${trade.taker_tx_hash.replace(/^0x/, "")}`}
              target="_blank" className="font-mono text-[14px]" style={{ color: "var(--accent-light)" }}>
              Taker TX ({trade.source_chain.includes("sepolia") ? "Sepolia" : "Supra"})
            </a>
          )}
          {trade.maker_tx_hash && (
            <a href={trade.dest_chain === "supra-testnet" ? `https://testnet.suprascan.io/tx/${trade.maker_tx_hash.replace(/^0x/, "")}` : `https://sepolia.etherscan.io/tx/${trade.maker_tx_hash}`}
              target="_blank" className="font-mono text-[14px]" style={{ color: "var(--accent-light)" }}>
              Maker TX ({trade.dest_chain.includes("supra") ? "Supra" : "Sepolia"})
            </a>
          )}
        </div>
      )}
...
export default function OrderbookTable({ rfqs, trades, quotes = [], agents = [], onAcceptQuote, onUpdate, hideInFlight, onlyInFlight }: Props) {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (bal) setMakerVault({ ...bal, availableCapacity: capData?.availableCapacity ?? Number(bal.matching_limit || 0), totalEarmarked: capData?.totalEarmarked ?? 0 });
    }).catch(() => {});
  }, [supraAddress]);
  ...
  useEffect(() => {
    ...
    supabase.from("committee_requests").select("trade_id, attestation_tx").in("trade_id", settledIds).eq("verification_type", "approve_reputation").not("attestation_tx", "is", null)
      .then(({ data }) => {
        if (data) {
  ...
  // Auto-expand trade when a tracked RFQ gets matched
  useEffect(() => {
    if (!trackedRfqId) return;
    const matchedTrade = activeTrades.find(t => t.rfq_id === trackedRfqId);
    if (matchedTrade && expandedTrade !== matchedTrade.id) {
      setExpandedTrade(matchedTrade.id);
      setTrackedRfqId(null);
    }
  }, [activeTrades, trackedRfqId]);
  ...
  const cancelRfq = async (rfqId: string) => {
    ...
    setCancelling(rfqId);
    try {
      const res = await fetch("/api/skill/suprafx", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ action: "cancel_rfq", rfqId, agentAddress: supraAddress }) });
      const data = await res.json();
      if (data.error) alert(data.error); else onAcceptQuote?.();
  ...
  const placeQuote = async (rfqId: string) => {
    ...
    try {
      ...
      try {
        ...
        quoteBody.signedPayload = signed.payload; quoteBody.signature = signed.signature; quoteBody.payloadHash = signed.payloadHash;
        quoteBody.sessionNonce = signed.payload.sessionNonce; quoteBody.sessionPublicKey = signed.sessionPublicKey;
        quoteBody.sessionAuthSignature = signed.sessionAuthSignature; quoteBody.sessionCreatedAt = signed.sessionCreatedAt;
      } catch (e) { console.warn("[SupraFX] Signing failed:", e); }
      const quoteRes = await fetch("/api/skill/suprafx", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(quoteBody) });
      const quoteData = await quoteRes.json();
      if (quoteData.error) { alert(quoteData.error); setQuotingLoading(false); return; }
      setQuotePrice(""); setQuotingRfq(null);
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
          ...
          <div>
            <div className="flex items-center gap-4 px-4 py-1.5" style={{ background: "var(--surface-2)", borderBottom: "1px solid var(--border)" }}>
              {["TX ID", "Pair", "Size", "Rate", "Route", "Taker", "Maker", "Status"].map(h => (
                <span key={h} className={"mono text-[10px] uppercase tracking-wider font-medium " + (h === "Rate" ? "flex-1" : ["TX ID","Pair","Size"].includes(h) ? (h === "TX ID" ? "w-24 shrink-0" : h === "Pair" ? "w-28 shrink-0" : "w-24 shrink-0") : "shrink-0")}
              ...
              return (
                <div key={t.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  <div className="flex items-center gap-4 px-4 py-3 transition-colors">
                    ...
                    <span className="text-[12px] shrink-0" style={{ color: "var(--t3)" }}>{t.source_chain} to {t.dest_chain}</span>
                    ...
                    <span className="shrink-0">
                      {t.maker_address === "auto-maker-bot"
                        ? <span className="mono text-[11px]" style={{ color: "var(--accent)" }}>SupraFX Bot</span>
      ...
      <div className="card mb-2 animate-in" style={{ order: 2 }}>
        ...
        {(myOpenRfqs.length > 0 || showRfqs) && (openRfqs.length === 0 ? (
          ...
          <div>
            <div className="flex items-center gap-4 px-4 py-1.5" style={{ background: "var(--surface-2)", borderBottom: "1px solid var(--border)" }}>
              {["TX ID", "Pair", "Size", "Asking", "Route", "Taker", "Quotes"].map(h => (
                <span key={h} className={"mono text-[10px] uppercase tracking-wider font-medium " + (h === "TX ID" ? "w-24 shrink-0" : h === "Pair" ? "w-28 shrink-0" : h === "Size" ? "w-24 shrink-0" : h === "Asking" ? "flex-1" : "shrink-0")}
                  style={{ color: "var(--t3)" }}>{h}</span>
              ))}
            </div>
              ...
              return (
                <div key={r.id} style={{ borderBottom: "1px solid var(--border)" }}>
                  <div className="flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-white/[0.01] transition-colors"
                    ...
                    <span className="mono text-[12px] w-24 shrink-0" style={{ color: "var(--t3)" }}>{generateTxId(r.display_id, r.taker_address)}</span>
                    <span className="text-[13px] font-semibold w-28 shrink-0">{pairClean}</span>
                    ...
                    <span className="text-[12px] shrink-0" style={{ color: "var(--t3)" }}>{r.source_chain} to {r.dest_chain}</span>
                    <div className="shrink-0"><AddrWithRep addr={r.taker_address} chain={r.source_chain} agents={agents} isMine={isMine} /></div>
                    <div className="flex items-center gap-2 shrink-0">
                  ...
                  {isExpanded && (
                    <div className="animate-slide-down" style={{ background: "var(--bg-raised)", borderLeft: "3px solid var(--border)" }}>
                      {rfqQuotes.length === 0 ? (
                        <div className="px-8 py-3 text-[13px]" style={{ color: "var(--t3)" }}>Waiting for maker quotes...</div>
                      ) : (
                        <div>
                          <div className="px-8 py-1.5 flex items-center gap-4" style={{ borderBottom: "1px solid var(--border)" }}>
                            {["Maker", "Quote Price", "USD Value", "You Receive", "vs Asking", "Status"].map(h => (
                              <span key={h} className={"mono text-[10px] uppercase tracking-wider font-medium " + (h === "Maker" ? "w-40" : h === "Quote Price" ? "w-36" : h === "USD Value" ? "w-28" : h === "You Receive" ? "w-32" : h === "vs Asking" ? "w-24" : "w-20")}
                                style={{ color: "var(--t3)" }}>{h}</span>
                            ))}
                          ...
                          {rfqQuotes.map(q => {
                            ...
                            return (
                              <div key={q.id} className="px-8 py-2.5 flex items-center gap-4 hover:bg-white/[0.02] transition-colors"
                                ...
                                <span className="mono text-[12px] w-24" style={{ color: diffColor }}>{diff >= 0 ? "+" : ""}{diff.toFixed(2)}%</span>
                                <span className="w-20">
                          ...
                          {quotingRfq === r.id ? (
                            <div className="flex items-center gap-3 flex-wrap">
                              ...
                              <input type="number" step="any" min="0" placeholder={fmtRate(r.reference_price)} value={quotePrice}
                                ...
                                style={{ background: "var(--bg)", color: "var(--t0)", border: "1px solid var(--border)", width: 160 }} autoFocus />
                              ...
                              {parseFloat(quotePrice) > 0 && r.size > 0 && (() => {
                                ...
                                return (
                                  <div className="text-[10px] mt-2 px-2 py-1.5 rounded w-full" style={{ background: "var(--surface-2)", color: "var(--t2)" }}>
                              ...
                              <button onClick={() => { setQuotingRfq(null); setQuotePrice(""); }}
                                className="text-[11px] hover:underline" style={{ color: "var(--t3)", background: "none", border: "none", cursor: "pointer" }}>
                                cancel
                              </button>
                            </div>
                          ) : (
                            <>
                              <button onClick={() => {
                                if (!makerVault || makerVault.matchingLimit <= 0) { setShowDepositPrompt(true); return; }
                                setQuotingRfq(r.id); setQuotePrice(fmtRate(r.reference_price)); setShowDepositPrompt(false);
                              }}
                              ...
                              {showDepositPrompt && quotingRfq !== r.id && (
                                <div className="mt-2 px-3 py-2.5 rounded" style={{ background: "var(--warn-dim)", border: "1px solid rgba(234,179,8,0.15)" }}>
                                  ...
                                  <div className="text-[12px] mb-2" style={{ color: "var(--t2)" }}>
                                    To place quotes, you need a security deposit in the vault. Your quoting capacity is 90% of your deposit.
                                  </div>
                                  <button onClick={() => { window.dispatchEvent(new Event("suprafx:open-vault")); setShowDepositPrompt(false); }}
          ...
          {showCompleted && (
            <div>
              ...
              {filteredCompletedTrades.map(t => {
                ...
                const pairClean = displayPair(t.pair);
                const baseClean = t.pair.split("/")[0]?.replace("fx", "") || "";
                const quoteClean = t.pair.split("/")[1]?.replace("fx", "") || "";
                  ...
                  <div key={t.id} style={{ borderBottom: "1px solid var(--border)" }}>
                    <div className="flex items-center gap-4 px-4 py-3 cursor-pointer hover:bg-white/[0.01] transition-colors"
                      ...
                      <span className={`tag tag-${t.status}`} style={
                        ...
                        {t.status === "settled" ? "Settled" : t.status === "taker_timed_out" ? "Taker Timed Out" : t.status === "maker_defaulted" ? "Maker Defaulted" : t.status.replace(/_/g, " ")}
                      </span>
                      <span className="text-[10px]" style={{ color: "var(--t3)" }}>{isExpanded ? "^" : "v"}</span>
                    </div>
                    ...
                    {isExpanded && (
                      <div className="px-6 pb-4 pt-1" style={{ background: "var(--bg-raised)", borderLeft: "3px solid var(--border)" }}>
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {t.taker_tx_hash && takerTxUrl
                                ? <a href={takerTxUrl} target="_blank" rel="noopener" className="mono text-[12px] hover:underline" style={{ color: "var(--accent)" }}>{t.taker_tx_hash.slice(0, 10)}...{t.taker_tx_hash.slice(-6)}</a>
                            ...
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              {t.maker_tx_hash && makerTxUrl
                                ? <a href={makerTxUrl} target="_blank" rel="noopener" className="mono text-[12px] hover:underline" style={{ color: "var(--accent)" }}>{t.maker_tx_hash.slice(0, 10)}...{t.maker_tx_hash.slice(-6)}</a>
                                : <span className="text-[12px]" style={{ color: "var(--t3)" }}>--</span>}
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-[11px] w-16" style={{ color: "var(--t3)" }}>Attestation:</span>
                              {att
                                ? <a href={`https://testnet.suprascan.io/tx/${att.replace(/^0x/, "")}`} target="_blank" rel="noopener" className="mono text-[12px] hover:underline" style={{ color: "var(--positive)" }}>{att.slice(0, 10)}...{att.slice(-6)}</a>
                                : t.status === "settled" ? <span className="text-[12px]" style={{ color: "var(--t3)" }}>pending</span>
                                : <span className="text-[12px]" style={{ color: "var(--t3)" }}>--</span>}
                            </div>
                            {t.settle_ms && (
                              <div className="flex items-center gap-2 mt-1">
                                <span className="text-[11px] w-16" style={{ color: "var(--t3)" }}>Duration:</span>
                                <span className="mono text-[13px] font-semibold" style={{ color: "var(--positive)" }}>{(t.settle_ms / 1000).toFixed(1)}s</span>
                              </div>
                            )}
                          </div>
                        </div>
                        ...
                        {tradeQuotes.length > 0 && (
                          <div>
                            <span className="mono text-[10px] uppercase tracking-wider block mb-1.5" style={{ color: "var(--t3)" }}>Quote History ({tradeQuotes.length})</span>
                            <div className="rounded overflow-hidden" style={{ border: "1px solid var(--border)" }}>
                              {tradeQuotes.map((q, qi) => {
                                ...
                                return (
                                  <div key={q.id} className="flex items-center gap-4 px-4 py-2"
                                    ...
                                    <div className="w-36"><AddrWithRep addr={q.maker_address} chain={t.dest_chain} agents={agents} isMine={q.maker_address === supraAddress} /></div>
                                    <span className="mono text-[13px] w-32" style={{ color: "var(--t1)" }}>{fmtRate(q.rate)} {quoteClean}</span>
                                    {qDiff !== null && <span className="mono text-[11px] w-20" style={{ color: qDiff >= 0 ? "var(--positive)" : "var(--negative)" }}>{qDiff >= 0 ? "+" : ""}{qDiff.toFixed(2)}%</span>}
