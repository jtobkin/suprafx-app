    ...
    <div className="rounded border overflow-hidden mb-4 animate-in"
      ...
      <div className="px-4 py-2.5 border-b flex items-center justify-between"
        ...
        <div className="flex items-center gap-3">
          <span className="text-[13px] font-medium" style={{ color: "var(--t1)" }}>Orderbook</span>
          {/* Toggle */}
          <div className="flex items-center rounded overflow-hidden border" style={{ borderColor: "var(--border)" }}>
            <button onClick={() => setView("global")}
              className="px-2.5 py-1 text-[13px] font-mono uppercase tracking-wider transition-colors"
              style={{
          ...
          <thead>
            <tr style={{ background: "var(--surface-2)" }}>
              {["ID","Pair","Size","Ref Price","Route","Taker","Slip","Status"].map(h => (
                <th key={h} className="px-4 py-2 text-left text-[13px] font-medium uppercase tracking-wider border-b"
            ...
            {all.map(r => {
              ...
              return (
                <tr key={r.id} className="transition-colors hover:bg-white/[0.01]"
                  ...
                  <td className="px-4 py-2 font-mono text-[14px]" style={{ color: "var(--t3)" }}>{r.display_id}</td>
                  <td className="px-4 py-2 text-[14px] font-semibold">{r.pair}</td>
                  <td className="px-4 py-2 font-mono text-[13px]">{r.size}</td>
                  <td className="px-4 py-2 font-mono text-[13px]">${Number(r.reference_price).toLocaleString(undefined, {minimumFractionDigits:2})}</td>
                  <td className="px-4 py-2 text-[14px]" style={{ color: "var(--t3)" }}>{r.source_chain} → {r.dest_chain}</td>
                  <td className="px-4 py-2 font-mono text-[14px]" style={{ color: isMine ? "var(--accent-light)" : "var(--t2)" }}>
                    {isMine ? "You" : r.taker_address.slice(0,10) + "…"}
                  </td>
                  <td className="px-4 py-2 font-mono text-[13px]" style={{ color: "var(--t2)" }}>{(Number(r.max_slippage)*100).toFixed(1)}%</td>
