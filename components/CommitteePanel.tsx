  ...
  return (
    <div className="rounded border overflow-hidden mb-4 animate-in"
      ...
      <div className="px-4 py-2.5 border-b flex items-center justify-between"
        ...
        <span className="text-[13px] font-medium" style={{ color: "var(--t1)" }}>Settlement Committee</span>
      ...
      <div className="flex gap-1 p-2.5 border-b" style={{ borderColor: "var(--border)" }}>
        {nodes.map(n => (
          <div key={n.id} className="flex-1 flex items-center justify-center gap-1.5 py-1.5 rounded"
            ...
            <span className="font-mono text-[13px] font-medium" style={{ color: "var(--t2)" }}>{n.id}</span>
          </div>
        ))}
      </div>
            ...
            return (
              <div key={r.id} className="border-b last:border-b-0" style={{ borderColor: "rgba(255,255,255,0.02)" }}>
                <div className="flex items-center px-4 py-2 cursor-pointer hover:bg-white/[0.01]"
                  ...
                  <div className="flex-1 text-[13px]" style={{ color: "var(--t2)" }}>
                    {r.verification_type.replace(/_/g, " ")}
                  </div>
                  <div className="font-mono text-[14px] mr-4" style={{ color: "var(--t3)" }}>
                    {r.trade_id.slice(0, 8)}…
                  </div>
                  <div className="font-mono text-[13px] mr-4">
                    {r.approvals}/{r.approvals + r.rejections}
                  </div>
                  <span className={`tag tag-${r.status}`}>{r.status}</span>
                  <span className="ml-2 text-[13px]" style={{ color: "var(--t3)" }}>
                    {isExpanded ? "▲" : "▼"}
                  </span>
                </div>
                ...
                {isExpanded && (
                  <div className="px-4 pb-3">
                    <div className="rounded p-2.5" style={{ background: "var(--bg)" }}>
                      <div className="text-[13px] font-mono uppercase tracking-wider mb-2" style={{ color: "var(--t3)" }}>
                        Node Signatures
                      </div>
                      {reqVotes.length > 0 ? reqVotes.map(v => (
                        <div key={v.id} className="flex items-center gap-2 py-1 border-b last:border-b-0"
                          style={{ borderColor: "rgba(255,255,255,0.03)" }}>
                          <span className="font-mono text-[14px] w-8" style={{ color: "var(--t2)" }}>{v.node_id}</span>
                          <span className={`tag tag-${v.decision === "approve" ? "approved" : "failed"}`}>
                            {v.decision}
                          </span>
                          {v.signature && (
                            <span className="font-mono text-[13px] truncate flex-1" style={{ color: "var(--t3)" }}>
                      ...
                      {reqVotes.length > 0 && reqVotes[0]?.signature && (
                        <div className="mt-2 pt-2 border-t" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                          <div className="text-[13px] font-mono uppercase tracking-wider mb-1" style={{ color: "var(--t3)" }}>
                            Aggregate Multisig
                          </div>
                          <div className="font-mono text-[13px] break-all" style={{ color: "var(--accent-light)" }}>
                            {(() => {
                          ...
                          {r.attestation_tx && (
                            <div className="mt-2 pt-2 border-t" style={{ borderColor: "rgba(255,255,255,0.04)" }}>
                              <div className="text-[13px] font-mono uppercase tracking-wider mb-1" style={{ color: "var(--positive)" }}>
